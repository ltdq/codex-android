...
val buildNative = tasks.register<Exec>("buildNative") {
    group = "build"
    description = "Build the embedded Codex app-server for Android."
    workingDir(repositoryRoot)
    commandLine("bash", "native/build.sh")
    // Cargo owns dependency tracking, including changes in the upstream workspace.
    val skipNativeBuild = providers.gradleProperty("skipNativeBuild").isPresent
...
val stageRuntime = tasks.register<StageRuntime>("stageRuntime") {
    ...
    doFirst {
        check(nativeLibrary.isFile && nativeHelper.isFile) {
            "Missing Codex JNI library. Run native/build.sh before packaging."
        }
        check(manifest.isFile) {
            "Missing toolchain package. Run toolchain/pack-jnilibs.sh arm64-v8a."
        }
    }
}
