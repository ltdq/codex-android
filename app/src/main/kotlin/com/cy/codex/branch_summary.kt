...
/** Mirrors `tui/src/branch_summary.rs`; git/gh run through `command/exec` with prompts disabled,
 * so a repo that needs credentials errors instead of hanging. */
...
/** The default branch to diff against: `origin/HEAD` first, then the usual names (branch_summary.rs). */
...
/** `gh pr view`: only an open pull request is reported. */
